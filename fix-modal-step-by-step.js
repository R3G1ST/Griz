...
console.log('🔧 Применяю изменения пошагово...\n');

// Шаг 1: Добавляем состояние после loading
if (!code.includes('fontModal')) {
  ...
  code = code.replace(loadingLine, newStates);
  console.log('✅ Шаг 1: Добавлены состояния');
}

// Шаг 2: Добавляем функции после update
if (!code.includes('openFontModal')) {
  const updateLine = 'const update = (key: string, value: any) => setData((prev: any) => ({ ...prev, [key]: value }));';
  const newFunctions = `const update = (key: string, value: any) => setData((prev: any) => ({ ...prev, [key]: value }));
  ...
  code = code.replace(updateLine, newFunctions);
  console.log('✅ Шаг 2: Добавлены функции');
}

// Шаг 3: Добавляем модальное окно перед закрывающим </div> функции SettingsTab
...
`;
    ...
    console.log('✅ Шаг 4: Добавлена CSS анимация');
  }
}
