      ...
      {editing && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-neutral-950 border border-white/10 p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto space-y-4">
            ...
            {/* Состав — только для bar, food, tea (НЕ для hookah) */}
            ...
            {/* Аллергены — только для bar, food, tea (НЕ для hookah) */}
            {editing.menuCategory && editing.menuCategory !== 'hookah' && (
              <div>
            ...
            {/* Кальян недели — ТОЛЬКО для hookah */}
            {editing.menuCategory === 'hookah' && (
                  ...
                  <div className="space-y-2 pt-2 border-t border-lime/20">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <input type="number" min="1" max="10" value={editing.strength || 4} onChange={e => setEdit({ ...editing, strength: Number(e.target.value) })} className={fieldClass} />
                      </div>
                      <div>
                        <label className="text-gray-400 text-xs">Время сессии (мин)</label>
                        <input type="number" value={editing.sessionDuration || 120} onChange={e => setEdit({ ...editing, sessionDuration: Number(e.target.value) })} className={fieldClass} />
                      </div>
                    </div>
                    <div>
                      <label className="text-gray-400 text-xs">Чаша</label>
                      <input value={editing.bowl || "Phunnel · Glaze"} onChange={e => setEdit({ ...editing, bowl: e.target.value })} className={fieldClass} />
                    </div>
                    <div>
                      <label className="text-gray-400 text-xs">Уголь</label>
                      <input value={editing.coal || ""} onChange={e => setEdit({ ...editing, coal: e.target.value })} className={fieldClass} />
                    </div>
                    <div>
                      <label className="text-gray-400 text-xs">Бренд табака</label>
                      <input value={editing.tobaccoBrand || ""} onChange={e => setEdit({ ...editing, tobaccoBrand: e.target.value })} className={fieldClass} />
                    </div>
                    <div>
                      <label className="text-gray-400 text-xs">Вкус табака</label>
                      <input value={editing.tobaccoFlavor || ""} onChange={e => setEdit({ ...editing, tobaccoFlavor: e.target.value })} className={fieldClass} />
                    </div>
                    <div>
                      <label className="text-gray-400 text-xs">Модель кальяна</label>
                      <input value={editing.hookahModel || ""} onChange={e => setEdit({ ...editing, hookahModel: e.target.value })} className={fieldClass} />
                    </div>
                    <div>
                      <label className="text-gray-400 text-xs">Цена (для кальяна недели)</label>
                      <input value={editing.priceFeatured || ""} onChange={e => setEdit({ ...editing, priceFeatured: e.target.value })} className={fieldClass} />
                    </div>
                    <div>
                      <label className="text-gray-400 text-xs">Описание (для кальяна недели)</label>
                      <textarea value={editing.descriptionFeatured || ""} onChange={e => setEdit({ ...editing, descriptionFeatured: e.target.value })} className={`${fieldClass} resize-none`} rows={3} />
                    </div>
                  </div>
                )}
              </div>
