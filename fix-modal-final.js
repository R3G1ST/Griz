...
console.log('🔧 Вставляю модальное окно...\n');

// Проверяем, есть ли функции
if (!code.includes('openFontModal')) {
  console.log('❌ Сначала добавь функции! Запусти fix-complete-safe.js');
  process.exit(1);
}
...
// Ищем строку с закрывающей скобкой перед "// ── Menu CMS tab"
const lines = code.split('\n');
let insertIndex = -1;

for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('// ── Menu CMS tab')) {
    // Нашли комментарий, идём назад до закрывающей скобки
    for (let j = i - 1; j >= 0; j--) {
      if (lines[j].trim() === '  );') {
        insertIndex = j;
        break;
...
if (insertIndex === -1) {
  console.log('❌ Не найдено место для вставки');
  process.exit(1);
}

// Вставляем модальное окно перед строкой с "  );"
lines.splice(insertIndex, 0, modalCode);
code = lines.join('\n');

fs.writeFileSync(adminPath, code);
console.log('✅ Модальное окно добавлено на строку ' + insertIndex);
console.log('\n🔥 Пересобери админку:');
