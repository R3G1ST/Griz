...
if (!code.includes('openFontModal')) {
  console.log('❌ Сначала нужно добавить функции! Запусти fix-complete-safe.js');
  process.exit(1);
}

// Находим конец SettingsTab - ищем последнюю строку перед закрывающим </div>
const lines = code.split('\n');
let insertIndex = -1;

// Ищем строку "return (" в SettingsTab
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('function SettingsTab()')) {
    // Нашли начало функции, теперь ищем конец
...
// Вставляем модальное окно перед закрывающим })
